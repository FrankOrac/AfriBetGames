...
export default function NightGame() {
  ...
  const { data: game } = useQuery<Game>({
    queryKey: ['/api/games', 'night'],
    queryFn: async () => {
      const response = await fetch('/api/games/night');
      if (!response.ok) throw new Error('Game not found');
      return await response.json() as Game;
    },
  });

  // Calculate time until next 12:00 AM (midnight) draw
  useEffect(() => {
    const calculateTimeUntilDraw = () => {
      const now = new Date();
      // Start from tomorrow's date, then set to midnight
  ...
  const placeBetMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      toast({
        ...
        description: `Your Night Game bet has been placed. Draw at 12:00 AM.`,
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-950 dark:from-black dark:to-gray-900">
      <Header />
      
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <Link href="/#games">
            <Button variant="ghost" className="gap-2 text-gray-200 dark:text-gray-300 hover:text-white" data-testid="button-back">
          ...
          <Card className="mb-6 bg-gradient-to-r from-indigo-600 to-purple-700 text-white border-0">
            ...
            <CardContent>
              <div className="bg-white/20 backdrop-blur rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-indigo-100 text-sm mb-1">Next Draw In</p>
                    <p className="text-3xl font-mono font-bold">{timeUntilDraw}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-indigo-100 text-sm mb-1">Odds Range</p>
          ...
          {!currentBetId && !generatedResult ? (
            <div className="grid gap-6">
              ...
              <Button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white py-6 text-lg font-semibold"
                data-testid="button-place-bet"
              >
                <Zap className="mr-2 h-5 w-5" />
                {placeBetMutation.isPending ? 'Placing Bet...' : `Place Night Bet - ₦${stakeAmount.toLocaleString()}`}
              </Button>
            </div>
          ) : (
            <div className="space-y-6">
              {currentBetId && !generatedResult && (
                <Card className="bg-indigo-50 dark:bg-indigo-950 border-2 border-indigo-500">
                  <CardHeader>
                    <CardTitle className="text-indigo-900 dark:text-indigo-100">✓ Bet Placed Successfully!</CardTitle>
                    <CardDescription className="text-indigo-700 dark:text-indigo-300">
                      Your Night Game bet has been recorded. Print your betting slip below.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <PrintTicket betId={currentBetId} autoOpen={false} />
                    <p className="text-sm text-indigo-600 dark:text-indigo-400 mt-4 text-center">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {JSON.parse(generatedResult.winningNumbers).map((num: number, idx: number) => (
                            <div key={idx} className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center font-bold text-indigo-700 dark:text-indigo-300">
                              {num}
                            </div>
                          ))}
                          {generatedResult.bonusNumber && (
                            <div className="w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900 flex items-center justify-center font-bold text-purple-700 dark:text-purple-300 border-2 border-purple-500">
                              {generatedResult.bonusNumber}
                            </div>
                          )}
                        </div>
